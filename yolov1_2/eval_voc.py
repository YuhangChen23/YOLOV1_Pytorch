...
def voc_eval(preds,target,VOC_CLASSES=VOC_CLASSES,threshold=0.5,use_07_metric=False,):
    ...
    for i,class_ in enumerate(VOC_CLASSES):
        ...
        print('---class {} ap {}---'.format(class_,ap))
        logfile.writelines('class: '+ str(class_) + '  ap: '+str(ap) + '\n')
        logfile.flush()
        aps += [ap]
    print('---map {}---'.format(np.mean(aps)))
    logfile.writelines('all map' + str(np.mean(aps)))
    logfile.flush()
...
if __name__ == '__main__':
    ...
    model = predict.resnet50()
    model.load_state_dict(predict.torch.load('./yolo_test4.pth'))
    model.eval()
    model.cuda()
    count = 0
    for image_path in tqdm(image_list):
        result = predict.predict_gpu(model,image_path,root_path='./data/val/') #result[[left_up,right_bottom,class_name,image_path],]
        for (x1,y1),(x2,y2),class_name,image_id,prob in result: #image_id is actually image_path
            preds[class_name].append([image_id,prob,x1,y1,x2,y2])
        #print(image_path)
        image = cv2.imread('./data/val/'+image_path)
        ...
        cv2.imwrite('./myimg/'+image_path,image)
#        count += 1
#        if count == 1:
#            break
